...
- (void)storeRTCOriginalAudioSetup {
    RTCAudioSession *rtcAudioSession = [RTCAudioSession sharedInstance];
    [rtcAudioSession lockForConfiguration];
    _origRTCAudioCategory = rtcAudioSession.category;
    _origRTCAudioCategoryOption = rtcAudioSession.categoryOptions;
    [rtcAudioSession unlockForConfiguration];
}

- (void)restoreRTCOriginalAudioSetup {
    
    RTCAudioSession *rtcAudioSession = [RTCAudioSession sharedInstance];
    NSError *error;
    [rtcAudioSession lockForConfiguration];
    if (![rtcAudioSession setCategory:self->_origRTCAudioCategory
                              withOptions:self->_origRTCAudioCategoryOption
                                    error:&error]) {
    NSLog(@"RNInCallManager. rtcAudioSession Error: %@",
                [error localizedDescription]);
        }
